...
<display:table name="newsList" cellspacing="0" cellpadding="0" requestURI=""
    id="newsList" pagesize="25" class="table newsList" export="true" defaultsort="4" defaultorder="descending" sort="list">
    ...
    <display:column escapeXml="true" sortable="true"
         titleKey="news.match">
			<c:choose>
					<c:when test="${newsList.match==0}"><fmt:message key="news.title"/></c:when>
					<c:when test="${newsList.match==1}"><fmt:message key="news.content"/></c:when>
				</c:choose>         
	</display:column>
         
    <display:column property="time" escapeXml="true" sortable="true"
         titleKey="news.time"/>
