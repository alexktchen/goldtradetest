...
public class Highlighter {
	 ...
	 *            re-parsing a document's text. Some work may be done on
	 *            retrieving TokenStreams more efficently by adding support for
	 *            storing original text position data in the Lucene index but
	 *            this support is not currently available (as of Lucene 1.4
	 *            rc2).
	 * @param text
	 *            text to highlight terms in
	 * 
	 ...
	 * @deprecated This method incorrectly hardcodes the choice of fieldname.
	 *             Use the method of the same name that takes a fieldname.
	 ...
	 *         of fragments)
	 */
	public final String[] getBestFragments(Analyzer analyzer, String fieldName,
	...
	public final TextFragment[] getBestTextFragments(TokenStream tokenStream,
		...
		try {
			...
			while ((token != null)
					&& (token.startOffset() < maxDocBytesToAnalyze)) {
			...
			// and that text is not too large...
				...
				newText.append(encoder
						.encodeText(text.substring(lastEndOffset)));
			}
