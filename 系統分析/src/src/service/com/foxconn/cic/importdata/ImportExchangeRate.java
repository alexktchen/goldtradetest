...
import com.foxconn.cic.model.ExchangeRateParser;
import com.foxconn.cic.service.ExchangeRateManager;
import com.foxconn.cic.service.ExchangeRateWebService;

public class ImportExchangeRate {

	private static ExchangeRateManager exchangeRateManager = null;

	private static ExchangeRateWebService searchWebService = null;

	private static String serviceURL = "";
	private static String exchangeRateServiceURL ="";
	private static String exchangeRateWebServiceURL="";

	public static void setServiceURL(String url) {
		serviceURL = url;
		exchangeRateServiceURL = serviceURL+"exchangeRate.service";
		exchangeRateWebServiceURL = serviceURL+"services/ExchangeRateWebService?wsdl";
	}
	...
	public static void init(String url) throws ParserConfigurationException, SAXException {
		...
		properties.addPropertyValue("serviceUrl", exchangeRateServiceURL);
		properties.addPropertyValue("serviceInterface",
				"com.foxconn.cic.service.ExchangeRateManager");
		...
		reg.registerBeanDefinition("exchangeRateManagerClient", definition);

		properties = new MutablePropertyValues();
		properties.addPropertyValue("serviceClass", "com.foxconn.cic.service.ExchangeRateWebService");
		properties.addPropertyValue("wsdlDocumentUrl",exchangeRateWebServiceURL);
		...
		exchangeRateManager = (ExchangeRateManager) factory.getBean("exchangeRateManagerClient");
		searchWebService = (ExchangeRateWebService) factory.getBean("searchWebService");
	...
	public static boolean isExisted(ExchangeRate rate,String condition){
		return searchWebService.isExisted(rate,condition);
	}

	public static boolean saveOrUpdate(Vector<ExchangeRate> rates,String condition){		
		for (ExchangeRate rate : rates) {
			if (isExisted(rate,condition)) {
				...
				try {
					newRate = exchangeRateManager.saveExchangeRate(rate);
	...
	public static boolean saveOrUpdateWithElement(Element element,String condition) throws DOMException, java.text.ParseException {
		Vector<ExchangeRate> rates = ExchangeRateParser.parse(element);
	...
	public static boolean saveOrUpdateWithString(String newsXML,String condition)
		...
		Vector<ExchangeRate> rates=ExchangeRateParser.parse(newsXML);
		return saveOrUpdate(rates,condition);
	}
}
